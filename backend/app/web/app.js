...
function renderFestivals() {
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    cells.push(`<article class="calendar-day${festivals.length ? ' has-festival' : ''}${isToday ? ' today' : ''}"><span>${day}</span>${festivals.map((festival) => `<strong class="festival-chip" title="${localizedTerm(festival.name)}">${localizedTerm(festival.name)}</strong>`).join('')}</article>`);
  }
